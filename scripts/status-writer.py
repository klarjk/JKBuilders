...
`/dev` 세션은 **자기 컨텍스트 포화도를 스스로 재서** 임계에 닿으면 정지·인수인계하고,
자동 이어가기 모드면 다음 회차를 예약한다. 그 계측의 유일한 재료가
`~/.claude/status/<session_id>.json`이고, 이 파일을 만드는 것은 클로드가 아니라
**사용자의 상태줄 명령**이다. 상태줄이 이 파일을 쓰지 않으면 세션은 자기 포화도를
못 재고, 못 재는 세션은 정지·회차 전환 시점을 놓친다.
