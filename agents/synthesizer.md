...
1. **architect 스폰** — Agent 도구에 `subagent_type="architect"`를 **반드시 명시**해 스폰한다. 설계 문제를 그대로 전달하고, **ADR을 스크래치패드 파일에 Write한 뒤 회신에는 경로만 담으라**고 지시한다. 회신 경로를 Read해 회수한다.
2. **critic 스폰** — Agent 도구에 `subagent_type="critic"`을 **반드시 명시**해 스폰한다. 1의 ADR 경로를 전달하고, 비평도 같은 방식(파일 Write + 경로 회신)으로 받는다.
...
자식 산출이 이쪽을 건너뛰고 호출자에게 배달될 수 있다. 회신이 오지 않으면 재요청하지 말고 지시한 파일 경로를 Read해 회수한다.
