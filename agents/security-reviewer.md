...
`cd <폴더> && <명령> <상대경로>` 꼴의 두 토막 명령을 쓰지 않는다. 파일은 **절대경로 한 토막**으로 지목하거나 전용 도구(Read·Grep·Glob)로 연다. 두 토막이면 읽을 파일이 실행 전에 확정되지 않아, 대상이 작업 폴더 안 소스여도 통과도 차단도 아닌 승인 요청으로 넘어가고 사람 없는 세션은 그 자리에 멈춘다. 읽기뿐 아니라 쓰기·출력 리다이렉션도 같다.

> **Do:** `grep -rn "apiKey" <프로젝트 절대경로>/src`
> **Don't:** `cd <프로젝트 절대경로>/src && grep -rn "apiKey" .`
