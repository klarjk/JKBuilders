...
log/             이벤트 메타
sessions/ (700)  상위도 700 — 열려 있으면 우편함 **이름 목록**이 보인다
sessions/<tmux세션명>/ (700)   notify-*.json (600) · *.sent · question-g<세대>-NN.json (600)
                        question-….json.answered (600 · 세션이 남기는 응답 표식)
...
청소는 **기동 시 1회 + 24시간 이상 연속 생존 시 하루 1회** 돈다(`mailbox.cleanup`). 지우는
것은 셋뿐이고, 기한은 모두 `max_age_days`(기본 7일)다.

| 대상 | 지우는 조건 | 기한을 세는 시각 |
|---|---|---|
| 알림 `notify-*.json` + `.sent` | 발신 완료 표식이 붙었다 | `.sent`의 mtime |
| **끝난 질문 교환** — `question-…json` + `.sent` + `.answered` + `answer-…json` | **응답 표식이 붙었다** | **`.answered`의 mtime** |
| 우편함 디렉토리 통째 | tmux가 그만큼 부재하고 미발신·보관분이 없다 | 최초 부재 관측 시각 |

**끝났다는 판정의 정본은 응답 표식이다.** 세션이 답을 받아 처리한 직후 자기 손으로 남긴
사실이라 흐려지지 않는다. **표식이 없는 질문은 나이를 묻지 않고 남는다** — 아직 답을
기다리는 질문을 오래됐다는 이유로 지우면 미답 질문이 조용히 사라지고, 사람은 자기가
답하지 않았다는 사실조차 모르게 된다.

기한을 표식 시각부터 세는 이유도 같다. 질문 본문의 mtime은 **발급 시각**이라, 하룻밤
넘겨 온 답을 받은 교환이 답을 처리한 바로 그날 지워질 수 있다.

> ⚠️ `max_age_days`는 **`answer_window`보다 길어야 한다.** 짧으면 아직 받아들이는 답이
> 왔을 때 열림 확인의 재료(질문 본문)가 이미 없어 멀쩡한 답이 `not_open`으로 되돌아간다.
> 기본값은 7일 대 1일이라 여유가 넉넉하다.

`pending-*.json`(보관분)과 창구 우편함(`sessions/counter/`)은 어느 규칙에도 걸리지 않는다.
...
**기동은 같은 점검을 스스로 한 번 돈다.** 주의가 1건이라도 나오면 창구 우편함
(`sessions/counter/`)에 `alert` 알림 파일을 남기고, 그 파일은 다른 알림과 똑같이 텔레그램으로
나간다 — **아무도 `--check`를 돌리지 않아도** 권한이 풀린 사실이 사람에게 닿는다. 주의가
0건이면 아무것도 쓰지 않는다(기동마다 "이상 없음"을 보내면 진짜 경고가 묻힌다). **앞선 통보가
아직 안 나갔으면 그때도 쓰지 않는다** — 창구 우편함은 청소 대상이 아니라, 못 고친 채 재기동을
되풀이하면 같은 통보가 그대로 쌓인다. 통보문의 경로는 홈 접두를 `~`로 접는다(통보는 제3자
서버에 남고, 받는 사람이 알 것은 열린 자리지 OS 사용자명이 아니다). 통보 쓰기가 실패해도
기동은 계속한다 — 통로를 여는 것이 통보보다 앞선다.
...
5. **점검한다.** `bot.py --check`가 fail-closed·권한·`never_send`를 한 번에 훑는다.
   **주의가 1건이라도 찍히면 고치고 다시 돌린다** — 종료코드는 주의가 있어도 0이라, 통과 여부는
   출력 마지막 줄의 건수로 읽는다. 건너뛰어도 기동이 같은 점검을 돌려 창구 우편함으로 알리지만,
   그때는 통로가 이미 열린 뒤다 — **띄우기 전에 한 번 돌리는 편이 낫다.**
...
정본은 저장소 루트 `CLAUDE.md`의 「테스트 실행 (파이썬 인터프리터)」 절이다 — 인터프리터·범위와
시스템 파이썬 3.9 임포트 확인 절차가 거기 있다. 우체부 몫은 다음 한 줄이고, **저장소 루트에서**
부른다.

```bash
/usr/local/bin/python3 -m pytest -q skills/postman
```

`tmux`도 텔레그램도 실호출하지 않는다.
...
```bash
install -d -m 700 ~/.claude/postman/sessions ~/.claude/postman/sessions/<자기 tmux명>
install -m 600 /dev/null <파일>     # 그 뒤 리다이렉션으로 채운다 — 권한은 그대로 남는다
```

**상위 `sessions/`도 같이 적는다.** macOS `install -d -m 700 a/b`는 잎(`b`)에만 모드를 주고 새로
생기는 상위(`a`)는 기본 권한으로 남긴다 — 그러면 안은 각 우편함 700으로 막혀도 **우편함 이름
목록이 남에게 보인다.** 이름은 곧 프로젝트·노드명이다.

자가 점검이 보는 자리는 넷이다 — 상위 `sessions/`(700) · 우편함 디렉토리(700) · 응답 표식(600) ·
**질문·알림 본문(600)**. 본문은 표식보다 민감하다(질문에는 사람에게 물으려던 문장이, 알림에는
세션이 내보낸 보고가 그대로 들어 있다). 어긋남은 **검출만 하고 고쳐 쓰지 않는다** — 쓰는 주체가
세션이라, 우체부가 권한까지 손대면 한 파일에 두 주체가 쓰게 된다.
...
못해, **그 통보는 창구가 이 우편함을 직접 훑어 가져간다** — 창구 우편함은 어느 청소 규칙에도
걸리지 않으므로 제어 통보에는 보존 기한이 없다(나간 것까지 재기동 횟수만큼 쌓인다).
통로 정본은 `skills/dev-loop/SKILL.md`의 「사용자에게 닿는 통로」다.
...
- 발신 뒤에도 **원본은 그대로 남는다**(`.sent` 표식만 붙는다). 하룻밤 뒤 온 답도 이 파일에서
  대조 재료를 얻는다. 원본이 사라지는 시점은 **응답 표식이 붙고 기한이 지난 뒤** 하나뿐이다
  (「무엇이 언제 사라지는가」).
...
표식을 남기면 그 교환은 **끝난 것으로 판정되어** 기한(`max_age_days`, 기본 7일) 뒤 질문
본문·`.sent`·`.answered`·`answer-…`가 한 묶음으로 사라진다. 표식이 없으면 질문은 나이와
무관하게 남는다 — 그러니 **답을 받기 전에 미리 남기지 않는다**(그 좌표의 답이 영영 안 들어가는
것과 같은 이유다).
