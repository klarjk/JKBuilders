...
| 구현 | 서브 | `/impl` |
| 실행 | 메인 | 1회성 조작 — 병렬 노드 합류(가역), 데이터 보충·이관·배포(비가역). 비가역이면 백업·복구 경로 확인이 통과 조건이고, 무인 진행 중이면 주체를 사용자로 되돌린다(`/dev-loop` `commander.md`의 "비가역 조작 경계") |
...
3. **성공 조건 (선택).** 사용자가 언급했을 때만 대화로 확정해 기록한다. 의도 인터뷰를 수행했으면 그 답변에서 확정한 것을 옮겨 적는다.
...
**반영은 어디에 적느냐까지가 규칙이다.** 계획서 본문에 적는 것은 **뒤 노드의 실행을 바꾸는 것**뿐이다 — 노드 표·흐름도의 칸, 전제·용어·성공 조건, 후속·유보 항목 한 줄. **그 노드가 무엇을 했고 무엇을 쟀는지의 서술은 길이를 불문하고 `nodes/<노드ID>.md`에 적고, 계획서에는 표의 `산출` 칸에 경로만 남긴다 — 주체를 가리지 않는다.** 메인이 직접 처리한 노드도 같다. 계획서는 노드를 하나 돌릴 때마다 통째로 다시 읽히므로, 본문에 눌러앉은 서술은 남은 모든 회차의 비용이 된다.
...
| 임계 | 산출 | 행동 |
|---|---|---|
| 약함 | `min(250000, 창 × 0.75)` | 새 작업 단위를 시작하지 않는다. 이미 시작한 것은 끝까지 회수·갱신한 뒤 정지·인수인계 |
| 강함 | `min(300000, 창 × 0.9)` | 진행 중인 것을 두고 나간다. 진행 상태 한 줄만 적립하고 즉시 정지 |

- **절대 토큰으로 계산하고 퍼센트를 쓰지 않는다.** 같은 상태 파일이 창 크기를 주므로 절대 토큰으로 직접 계산하는 편이 정확하다.
- **상한을 그대로 상수로 박지 않는다.** 창이 200K로 내려오면 고정 상한은 창 전체에 가까워져 닿는 순간 이미 자연사한 뒤이고 강한 임계는 영영 울리지 않는다 — 값도 코드도 정상으로 보여 판정이 죽었다는 사실조차 드러나지 않는다. **창 200K~333K는 상한이 걸리지 않아 계수만으로 값이 정해진다.**
- **약한 상한 250,000은 주행거리를 버는 값이다.** 상한이 200,000이면 한 세션이 기동 바닥 위에서 11만 남짓 일하는데 250,000이면 16만 안팎이 되어, 정지·인수인계 횟수와 회차마다 새로 까는 바닥이 줄어든다. 약한↔강한 간격은 창 1M에서 50,000으로 좁아지지만 실측 소진폭 30,000의 1.7배가 남고, 인수인계를 상시 적립하므로 강한 임계를 넘어도 유실은 없다.
...
**남기는 것.** 프론트매터, `## 목표`·`## 의도`·`## 사용자 여정`·`## 용어`·`## 성공 조건`·전제 절·`## 아키텍처 결정`, 흐름도, **노드 표는 한 행도 빼지 않는다**, 열린 후속·유보 항목, 대기 노드에 적힌 실행 지시, 진행 노드의 직전 회차.
