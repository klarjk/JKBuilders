...
- **컨텍스트 예산은 메인 세션에 쌓인 입력 토큰의 절대량으로 관리한다** — 현재는 Phase 3 묶음 경계에만 적용("컨텍스트 임계 판정").
...
1. '위임 실행 모드' 조건에 해당하지 않는 호출에서 `--followup`이 있거나 사용자가 밀린 후속 작업 처리를 요청하면 → **후속 소진(Phase 3)**. 아래 2~4번을 적용하지 않는다.
2. `--resume <경로>`가 있으면 그 계획서를 사용한다. 없으면 `docs/dev/plan/`을 스캔한다.
3. `status: in-progress` 계획서가 있으면 → **재진입(Phase 2)**. 2개 이상이면 제목을 1줄씩 보여주고 어느 것을 이어갈지 질문 후 답 대기.
4. 없으면 → **신규(Phase 1)**.
...
3. 사용자에 1줄 보고 후 진행 ("실행 가능: 007-N4 구현·서브, 007-N6 조사·서브. 병렬로 진행합니다").
4. 주체별 처리:
...
7. **후속·유보 처리** ("후속·유보 작업"). 전 노드가 완료·건너뜀이면 `status: done`으로 바꾸고, 후속 작업에 `[ ]` 항목이 남아 있으면 완료 보고에 함께 출력하며 `/dev --followup`으로 소진할 수 있음을 1줄 안내한다.
8. **세션 경계 판단** ("세션 경계 규칙").
...
메인 세션에 쌓인 **입력 토큰 절대량**으로 판정한다. 창은 100만이지만 비용과 판단 품질 때문에 20만에서 끊는다.

```bash
jq -r '.context_window.total_input_tokens' ~/.claude/status/${CLAUDE_CODE_SESSION_ID}.json
```

- **임계 200,000토큰.** 넘었으면 **새 묶음을 시작하지 않는다.** 이미 시작한 묶음은 끝까지 회수·갱신한 뒤 정지·인수인계한다.
- 검사 자리는 **묶음 경계 한 곳**이다 — 첫 묶음 시작 직전, 그 뒤로는 묶음 완료 직후. 완료 직후와 다음 묶음 시작 직전은 같은 시점이므로 두 번 재지 않고, 묶음이 도는 중에도 재지 않는다.
- 판정은 숫자 비교다. 퍼센트 필드(`used_percentage`)는 1%가 1만 토큰이라 해상도가 모자라므로 쓰지 않는다.
- 상태 파일이 없거나 읽히지 않으면 임계 판정을 건너뛰고 '세션 경계 규칙'의 무게 판정만 쓴다.
- 적용 지점은 **Phase 3의 묶음 경계**다. Phase 2 노드 경계에는 아직 적용하지 않는다.
