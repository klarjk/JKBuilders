...
`FOLLOWUP.md`가 이미 있으면 수확을 건너뛰고 묶음 표에서 상태가 `완료`가 아닌 첫 묶음부터 잇는다. 단 대장의 수확일이 7일 이상 지났으면 다시 수확할지 질문 후 답 대기.
...
- **항목별 판정 4종** — ① **유효성**: 코드·커밋과 대조해 이미 반영됐거나 폐기됐는지 ② **성격**: 코드 수정 / 결정 대기 / 수동 검증 / 배포·운영 / 문서 / 조사 ③ **착지 파일**: 항목에 없으면 코드에서 찾아 보완 ④ **의존**: 선행 항목과 같은 파일·화면을 건드리는 항목.
...
3. 남은 코드 수정 항목만 3번의 편성 대상이다.

### 3. 묶음 편성 (메인 직접)

1. **클러스터** — 같은 파일·모듈·화면을 건드리는 항목끼리 묶는다. 성격이 다른 항목은 한 묶음에 섞지 않는다.
...
편성 결과를 대장에 묶음 표(`묶음ID | 항목 | 착지 파일 | 출처 계획서 | 상태(대기/진행/완료)`)로 적고 사용자에 묶음 수와 첫 묶음만 1줄 보고한다.

### 4. 실행·갱신

묶음 하나를 `/impl`에 위임한다 — 전달: 묶음 항목 원문 · 착지 파일 · 출처 계획서 경로. 반환 후 출처 계획서의 해당 항목을 닫고 대장의 묶음 상태를 갱신한다.

'세션 경계 규칙'을 그대로 적용하되, `/impl`이 크기 중·대로 처리한 묶음은 무거움으로 본다.
