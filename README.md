...
`/dev-loop` 계열은 1번과 3번이 갖춰져야 돕니다(2번은 선택). 나머지 계열(`/prp`·메모리)은 1번만 있으면 됩니다.
...
### 2. 텔레그램 연결 (선택 — 우체부를 쓸 때만)

우체부(`skills/postman/`)는 Claude 세션의 질문을 텔레그램으로 보내고 답을 그 세션 화면에 되넣는 **독립 중계 프로그램**입니다. `/dev-loop` 계열이 요구하는 구성 요소가 아니며, **설치하지 않아도 `/dev-loop`는 그대로 돕니다** — 질문이 화면으로만 오고, 사용자가 자리에 있어야 할 뿐입니다.
...
`/dev-loop`는 노드를 맡을 작업 세션을 tmux로 띄웁니다. `tmux`가 없으면 스폰 전 검사에서 멈춥니다. `/dev`·`/impl` 단독 사용에는 필요 없습니다.
