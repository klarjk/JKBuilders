...
Claude Code에서 소프트웨어를 만들 때 쓰는 **워크플로우 도구 모음**입니다. 개발 계획을 세우고, 설계를 결정하고, 코드를 구현·테스트·리뷰하는 과정을 여러 전문 에이전트와 스킬이 나눠 맡습니다. 명령 한 줄(`/dev`, `/impl`, `/prp-plan` 등)이면 뒤에서 관련 에이전트들이 자동으로 협업합니다.
...
동시에 굴리는 구현 칸은 **칸마다 격리된 작업 공간과 별도 Claude 세션**에 넘깁니다. 도는 세션들은 한 창에 나란히 비춰 볼 수 있고, 답이 필요한 칸은 더블클릭해 바로 끼어들 수 있습니다.

tmux 안에서 `--auto`와 종료 조건(시각 또는 도달할 칸)을 붙이면 **자동 이어가기**가 켜집니다. 사람이 답해야 할 질문을 첫 회차에 한꺼번에 모아 묻고, 그 뒤로는 대화 용량이 찰 때마다 스스로 새 회차를 이어 붙이며 종료 조건까지 갑니다. 도중에 만나는 선택지는 판정 에이전트가 대신 고르고, "더 못 간다"는 판단도 심판 에이전트가 한 번 더 심사합니다.
...
| 갈래 | 하는 일 | 진입 명령 |
|------|---------|-----------|
| **/dev 계열** | 멀티세션 개발 지휘 — 할 일을 작업 흐름도로 배치해 조사·설계·구현·리뷰를 진행. 병렬 칸은 별도 세션에 위임하고, 원하면 종료 조건까지 자동으로 이어감 | `/dev` · `/impl` · `/adr` |
...
```
JKBuilders의 /dev 계열만 내 시스템에 이식해줘.
skills/dev · impl · adr · tdd-workflow 와 이들이 스폰하는
...
`/dev` 계열의 병렬 위임·자동 이어가기는 아래 둘이 모두 갖춰져야 돕니다. 나머지 계열(`/prp`·메모리)은 1번만 있으면 됩니다.
...
- **`statusLine`** — 단순한 화면 장식이 아니라 **`/dev`의 계측 배선**입니다. 세션이 자기 대화 용량을 재는 재료가 여기서 나오고, 이 값이 없으면 정지·인수인계 시점과 자동 이어가기의 회차 전환 시점을 놓칩니다. 이미 쓰는 상태줄 스크립트가 있다면 `scripts/status-writer.py` 머리말의 **②번 방식**으로 네 줄만 얹으세요.

### 2. tmux

`/dev`는 병렬 구현 칸의 작업 세션을 tmux로 띄우고, 자동 이어가기도 tmux 창 안에서만 켜집니다. `tmux`가 없으면 병렬 칸을 별도 세션에 넘기지 못해 메인 세션이 직접 처리하고, 자동 이어가기는 켜지지 않습니다. `/impl` 단독 사용에는 필요 없습니다.
...
- **자동 이어가기는 승인 창 없이 돕니다.** 사람이 없는 동안 멈추지 않도록 작업 세션을 권한 확인 생략 모드로 띄웁니다. 믿을 수 있는 저장소에서만 켜세요.
